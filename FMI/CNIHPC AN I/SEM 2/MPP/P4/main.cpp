...
void localMultiply(int n, const vector<matrix_t>& A, const vector<matrix_t>& B, vector<matrix_t>& C) {
    for (int i = 0; i < n; ++i) {
        for (int k = 0; k < n; ++k) {
...
void readParallel(const string& filename, vector<matrix_t>& local_mat, uint32_t M, int n, int rank, MPI_Comm cart_comm) {
    ...
    MPI_File file;
    MPI_File_open(cart_comm, filename.c_str(), MPI_MODE_RDONLY, MPI_INFO_NULL, &file);
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 5) {
        if (world_rank == 0) cout << "Usage: mpirun -np <P> " << argv[0] << " <M> <FileA> <FileB> <FileC>" << endl;
    ...
    // Pas 2: Main loop (q steps)
    for (int step = 0; step < q; ++step) {
        localMultiply(n, localA, localB, localC);
