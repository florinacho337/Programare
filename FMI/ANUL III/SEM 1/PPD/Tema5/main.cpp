...
void workerFunction(SharedQueue& sharedQueue, const LinkedList& leaderboard, const atomic<bool>& doneReading) {
    while (true) {
        pair<int, int> item;
        {
            unique_lock lock(sharedQueue.queueMutex);
            sharedQueue.conditionNotEmpty.wait(lock, [&sharedQueue, &doneReading]() {
                return !sharedQueue.q.empty() || doneReading;
            });

            if (sharedQueue.q.empty() && doneReading) {
                lock.unlock();
                return;
            }
        ...
        leaderboard.insertOrUpdate(item.first, item.second, item.first / 100); // ID / 100 pentru a obține țara
    }
}
