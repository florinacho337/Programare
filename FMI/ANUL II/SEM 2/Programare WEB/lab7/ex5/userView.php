...
function printUser($id, $username) {
    $target_dir = $_SERVER['DOCUMENT_ROOT'] . "/ex5/photos/" . $id . "/";
    ...
    if(count($images)) {
        ...
        foreach($images as $image) {
            ...
            echo '<img style="height: 100px;width: 100px" src="http://localhost/ex5/photos/' . $id . '/' . basename($image) . '"/><br/>';
            echo '</section>';
        }
        echo"</div></div>";
    }
...
$dir = "/5/photos/" . $_SESSION['id'] . "/";
$images = glob($target_dir . "*.{png,jpg,jpeg}", GLOB_BRACE);

echo '<div style="display: flex;flex-direction: row">';
foreach($images as $image) {
    echo '<section style="display:flex;flex-direction: column;max-width:100px;margin-left: 10px">';
    echo '<img style="height: 100px;width: 100px" src="http://localhost/ex5/photos/' . $_SESSION['id'] . '/' . basename($image) . '"/><br/>';
    echo '<button onclick="window.location.href=\'./deletePhoto.php?filename='. $image . '\'">Delete';
    echo '</button>';
    echo '</section>';
}

$mysqli = new mysqli("localhost", "root", "password", "useri");
if($mysqli->connect_error) {
  exit('Could not connect');
}

$sql = 'SELECT id, username FROM useri WHERE id != ?';
